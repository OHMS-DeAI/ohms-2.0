...
use ohms_adaptq::{NOVAQConfig, NOVAQModel, PublicNOVAQ, VerbosityLevel};
    ...
    for model in &models {
        ...
        for (bits, subspaces) in &configs {
            ...
            for i in 0..iterations {
                ...
                match benchmark_model_config(model, *bits, *subspaces, verbosity_level) {
                    Ok(result) => {
                        ...
                        println!(
                            "    ✅ Completed - {:.1}x compression, {:.1}% accuracy",
            ...
            if !config_results.is_empty() {
                let avg_result =
                    calculate_average_results(&config_results, model, *bits, *subspaces);
                all_results.push(avg_result);

                // Save individual results
                let config_file = output_path.join(format!(
                    "{}_{}bit_{}sub_{}.json",
                    model.replace("/", "_"),
                    bits,
                    subspaces,
...
fn compare_quantization_settings(
    model: &str,
    output: &str,
) -> Result<(), Box<dyn std::error::Error>> {
...
fn generate_benchmark_report(
    ...
    if all_results.is_empty() {
        ...
        let avg_compression_ratio: f64 = all_results
            .iter()
            .map(|r| r.performance.compression_ratio)
            .sum::<f64>()
            / all_results.len() as f64;

        let avg_accuracy: f64 = all_results
            .iter()
            .map(|r| r.quality.bit_accuracy)
            .sum::<f64>()
            / all_results.len() as f64;

        let avg_quality_score: f64 = all_results
            .iter()
        ...
        report.push_str(&format!(
            "- **Average Compression Ratio**: {:.1}x\n",
            avg_compression_ratio
        ));
        report.push_str(&format!(
            "- **Average Bit Accuracy**: {:.1}%\n",
            avg_accuracy * 100.0
        ));
        report.push_str(&format!(
        ...
        sorted_by_compression.sort_by(|a, b| {
            b.performance
                .compression_ratio
                .partial_cmp(&a.performance.compression_ratio)
                .unwrap()
        });
        ...
        for (i, result) in sorted_by_compression.iter().take(5).enumerate() {
            report.push_str(&format!(
                ...
                i + 1,
                result.model_name,
                result.config.target_bits,
                result.config.num_subspaces
            ));
            report.push_str(&format!(
                "   - Compression: {:.1}x\n",
                result.performance.compression_ratio
        ...
        sorted_by_accuracy.sort_by(|a, b| {
            b.quality
                .bit_accuracy
                .partial_cmp(&a.quality.bit_accuracy)
                .unwrap()
        });
...
fn benchmark_model_config(
    ...
    // Compress model
    let compressed_model = novaq.compress_hf_model(model, None).map_err(|e| {
    ...
    let validation = novaq.validate_model(&compressed_model).map_err(|e| {
        Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            e.to_string(),
        ))
...
fn calculate_average_results(
    results: &[BenchmarkResult],
    model: &str,
    bits: f32,
    subspaces: u32,
) -> BenchmarkResult {
    let avg_compression_time = results
        .iter()
        .map(|r| r.performance.compression_time_seconds)
        .sum::<f64>()
        / results.len() as f64;
    let avg_compression_ratio = results
        .iter()
        .map(|r| r.performance.compression_ratio)
        .sum::<f64>()
        / results.len() as f64;
    let avg_memory_usage = results
        .iter()
        .map(|r| r.performance.memory_usage_mb)
        .sum::<f64>()
        / results.len() as f64;
    let avg_throughput = results
        .iter()
        .map(|r| r.performance.throughput_mb_per_second)
        .sum::<f64>()
        / results.len() as f64;

    let avg_bit_accuracy =
        results.iter().map(|r| r.quality.bit_accuracy).sum::<f64>() / results.len() as f64;
    let avg_quality_score =
        results.iter().map(|r| r.quality.quality_score).sum::<f64>() / results.len() as f64;
