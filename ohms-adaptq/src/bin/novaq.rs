...
use ohms_adaptq::{CompressionStats, NOVAQConfig, PublicNOVAQ};
...
fn compress_hf_model(
    ...
    let model = novaq.compress_hf_model(repo, file).map_err(|e| {
...
fn compress_ollama_model(
    ...
    let model = novaq.compress_ollama_model(model).map_err(|e| {
        Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            e.to_string(),
        ))
...
fn compress_url_model(
    ...
    let model = novaq.compress_url_model(url, None).map_err(|e| {
        Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            e.to_string(),
        ))
...
fn compress_local_model(
    ...
    let model = novaq
        .compress_local_model(path.to_str().unwrap())
        .map_err(|e| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
...
fn validate_model(path: &PathBuf) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let model: ohms_adaptq::NOVAQModel = bincode::deserialize(&model_data).map_err(|e| {
        Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            e.to_string(),
        ))
    ...
    let validation = novaq.validate_model(&model).map_err(|e| {
        Box::new(std::io::Error::new(
            std::io::ErrorKind::Other,
            e.to_string(),
        ))
...
fn show_compression_results(
    ...
    println!(
        "{}",
        Blue.paint("🎉 Model successfully compressed with NOVAQ!")
    );
    println!(
        "{}",
        Blue.paint("   No restrictions, no gatekeeping - pure democratic access.")
    );
