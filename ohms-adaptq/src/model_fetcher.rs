...
impl ModelSource {
    ...
    pub fn to_url(&self) -> anyhow::Result<String> {
        match self {
            ...
            ModelSource::Ollama { model } => Err(anyhow::anyhow!(
                "Ollama models require local pull first: ollama pull {}",
                model
            )),
            ModelSource::LocalPath { .. } => Err(anyhow::anyhow!("Local paths don't have URLs")),
        }
    }
}
...
impl ModelFetcher {
    pub fn fetch(source: &ModelSource) -> anyhow::Result<FetchResult> {
        match source {
            ...
            ModelSource::HuggingFace { repo, file } => {
                fetch_hf_with_metadata(repo, file.as_deref())
            }
...
fn fetch_hf_with_metadata(repo: &str, file: Option<&str>) -> anyhow::Result<FetchResult> {
    ...
    Err(last_err.unwrap_or_else(|| {
        anyhow::anyhow!(
            "no compatible weight file found in repo '{}'; specify :<file>",
            repo
        )
    }))
}
...
fn try_hf_cli_download(repo: &str, file: Option<&str>) -> anyhow::Result<Option<PathBuf>> {
    ...
    let which = Command::new("bash")
        .arg("-lc")
        .arg("command -v huggingface-cli")
        .output()?;
    if which.status.code().unwrap_or(1) != 0 {
        return Ok(None);
    }
    ...
    if let Some(f) = file {
        ...
        args.push("--include".into());
        args.push("model.safetensors".into());
        args.push("--include".into());
        args.push("consolidated.safetensors".into());
        args.push("--include".into());
        args.push("pytorch_model.bin".into());
    }
