...
impl KnowledgeDistillationEngine {
    ...
    pub fn apply_distillation_loss(
        ...
        let soft_loss = self
            .compute_kl_divergence_loss(student_outputs, &teacher_knowledge.output_distributions);
...
mod tests {
    ...
    fn test_distillation_loss() {
        ...
        let loss = engine
            .apply_distillation_loss(&student_outputs, &teacher_knowledge, &hard_targets)
            .unwrap();
        assert!(loss > 0.0);
