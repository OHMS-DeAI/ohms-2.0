use super::{NormalizationMetadata, WeightMatrix};
use crate::Result;
...
impl DistributionNormalizer {
    ...
    fn identify_outlier_channels(&self, variances: &[f32]) -> Vec<usize> {
        let mut indexed_variances: Vec<(usize, f32)> =
            variances.iter().enumerate().map(|(i, &v)| (i, v)).collect();
...
impl ChannelStatistics {
    ...
    pub fn print_summary(&self) {
        ...
        println!(
            "  Max variance: {:.6}",
            self.variances.iter().fold(0.0f32, |a, &b| a.max(b))
        );
        println!(
            "  Min variance: {:.6}",
            self.variances.iter().fold(f32::INFINITY, |a, &b| a.min(b))
        );
...
mod tests {
    ...
    fn test_denormalization() {
        ...
        normalizer
            .denormalize(&mut denormalized, &metadata)
            .unwrap();

        // Should be close to original
        for (orig, denorm) in data.iter().zip(denormalized.iter()) {
            assert!(
                (orig - denorm).abs() < 1e-5,
