...
impl QuantizationProgressTracker {
    ...
    pub fn start_phase(&mut self, phase: QuantizationPhase, max_iterations: Option<u64>) {
        ...
        self.main_progress
            .set_message(format!("{} {}", phase_name, description));
        ...
        if matches!(
            ...
            println!(
                "{}",
                Blue.bold().paint(format!("🔄 Starting: {}", phase_name))
            );
        }
    }
    ...
    pub fn complete_phase(&mut self) {
        ...
        if matches!(
            ...
            let status_icon = if self.quality_metrics.recovery_count > 0 {
                "🛡️"
            } else {
                "✅"
            };
    ...
    pub fn complete(&mut self) {
        ...
        if matches!(
            ...
            println!(
                "  Final Accuracy: {:.2}%",
                self.quality_metrics.accuracy * 100.0
            );
    ...
    pub fn error(&mut self, error_msg: &str) {
        self.phase_progress
            .abandon_with_message(format!("❌ {}", error_msg));
        self.main_progress
            .abandon_with_message("❌ Quantization failed");
    ...
    fn get_phase_info(&self, phase: QuantizationPhase) -> (&str, &str) {
        match phase {
            QuantizationPhase::ModelLoading => {
                ("Model Loading", "Loading and preprocessing model weights")
            }
            QuantizationPhase::CodebookInitialization => {
                ("Codebook Init", "Initializing L1 and L2 codebooks")
            }
            QuantizationPhase::Level1Refinement => {
                ("L1 Refinement", "Optimizing level-1 codebook centroids")
            }
            QuantizationPhase::Level2Refinement => {
                ("L2 Refinement", "Optimizing level-2 residual codebooks")
            }
            QuantizationPhase::QualityValidation => {
                ("Quality Check", "Validating compression quality")
            }
