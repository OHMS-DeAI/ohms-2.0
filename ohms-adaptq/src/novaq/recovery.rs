use super::{
    NOVAQConfig, NOVAQEngine, NOVAQModel, NumericalStabilityGuard, QualityMetrics,
    QuantizationPhase, QuantizationProgressTracker, VerbosityLevel, WeightMatrix,
};
use crate::Result;
...
impl QuantizationRecoveryManager {
    ...
    pub fn quantize_with_recovery_and_progress(
        &mut self,
        ...
        match self.attempt_quantization_with_progress(
            &weights,
        ...
        for attempt in 1..=self.max_recovery_attempts {
            ...
            if !self
                ...
                .contains(&analysis.failure_type)
            {
                self.recovery_stats
                    .failure_types_encountered
                    .push(analysis.failure_type.clone());
            }
            ...
            for strategy in &analysis.recommended_strategies {
                ...
                println!(
                    "🛠️  Attempting recovery strategy: {:?} (attempt {}/{})",
                ...
                match self.attempt_quantization(&weights, &recovered_config) {
                    Ok(model) => {
                        ...
                        println!(
                            "✅ Quantization recovered successfully after {} attempts in {:.1}s",
                            attempt,
        ...
        println!(
            "💥 All recovery attempts exhausted after {:.1}s",
            total_duration.as_secs_f32()
        );
        self.print_recovery_summary();

        Err(format!(
            "Quantization failed after {} recovery attempts. Last error: {}",
    ...
    fn attempt_quantization_with_progress(
        &mut self,
        weights: &[WeightMatrix],
        config: &NOVAQConfig,
    ...
    fn analyze_error(
        ...
        let (failure_type, confidence) =
            if error_lower.contains("nan") || error_lower.contains("inf") {
            ...
            } else if error_lower.contains("bounds")
    ...
    fn apply_recovery_strategy(
        ...
        match strategy {
            ...
            RecoveryStrategy::FallbackToSimpleQuantization => {
                ...
                println!(
                    "   📉 Falling back to simple quantization (1 subspace, reduced codebooks)"
                );
            }
            RecoveryStrategy::ReduceCodebookSizes => {
                ...
                println!(
                    "   📊 Reducing codebook sizes to L1={}, L2={}",
    ...
    fn record_successful_recovery(
        ...
        let total_time = self.recovery_stats.average_recovery_time.as_secs_f32()
            * (self.recovery_stats.successful_recoveries - 1) as f32
            + duration.as_secs_f32();
        self.recovery_stats.average_recovery_time =
            Duration::from_secs_f32(total_time / self.recovery_stats.successful_recoveries as f32);
    ...
    pub fn print_recovery_summary(&self) {
        ...
        if self.recovery_stats.successful_recoveries > 0 {
            let success_rate = (self.recovery_stats.successful_recoveries as f32
                / self.recovery_stats.total_attempts as f32)
                * 100.0;
            println!("   Success rate: {:.1}%", success_rate);
            println!(
                "   Average recovery time: {:.1}s",
                self.recovery_stats.average_recovery_time.as_secs_f32()
            );
        ...
        if !self.recovery_stats.failure_types_encountered.is_empty() {
            println!(
                ...
                self.recovery_stats.failure_types_encountered
            );
        }
    }
...
mod tests {
    ...
    fn test_error_analysis_nan() {
        ...
        assert!(analysis
            .recommended_strategies
            .contains(&RecoveryStrategy::IncreaseStabilityThresholds));
    }
    ...
    fn test_recovery_strategy_application() {
        ...
        let recovered_config =
            manager.apply_recovery_strategy(RecoveryStrategy::ReduceLearningRate, &analysis);
    ...
    fn test_fallback_strategy() {
        ...
        let recovered_config = manager
            .apply_recovery_strategy(RecoveryStrategy::FallbackToSimpleQuantization, &analysis);
