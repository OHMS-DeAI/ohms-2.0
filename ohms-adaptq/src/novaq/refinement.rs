use super::{NumericalStabilityGuard, QuantizationIndices, VectorCodebooks, WeightMatrix};
use crate::Result;
...
impl TeacherGuidedRefiner {
    ...
    pub fn refine(
        ...
        for iteration in 0..self.max_iterations {
            ...
            let student_weights =
                self.reconstruct_from_codebooks(codebooks, indices, original_weights)?;
            ...
            let cosine_loss =
                self.compute_cosine_similarity_loss(teacher_outputs, &student_outputs);
    ...
    fn reconstruct_from_codebooks(
        ...
        for row_idx in 0..rows {
            for subspace_idx in 0..num_subspaces {
                ...
                for (offset, (&l1_val, &l2_val)) in
                    l1_centroid.iter().zip(l2_centroid.iter()).enumerate()
                {
    ...
    fn compute_cosine_similarity_loss(
        &self,
        teacher_hidden: &[f32],
        student_hidden: &[f32],
    ) -> f32 {
        ...
        let dot_product: f32 = teacher_hidden
            .iter()
        ...
        for subspace_idx in 0..num_subspaces {
            ...
            for centroid_idx in 0..codebooks.level1_codebooks[subspace_idx].len() {
                ...
                if !assigned_vectors.is_empty() {
                    let centroid =
                        &mut codebooks.level1_codebooks[subspace_idx][centroid_idx].centroid;
                    for dim in 0..centroid.len() {
                        let values: Vec<f32> = assigned_vectors
                            .iter()
                            .map(|v| if dim < v.len() { v[dim] } else { 0.0 })
                            .collect();

                        let mean = self.stability_guard.safe_mean(&values);
                        let new_val = self.stability_guard.safe_weighted_update(
                            centroid[dim],
                            mean,
                            adaptive_lr,
                        );
            ...
            for centroid_idx in 0..codebooks.level2_codebooks[subspace_idx].len() {
                ...
                for row_idx in 0..rows {
                    if indices.level2_indices[row_idx][subspace_idx] == centroid_idx as u8 {
                        ...
                        let l1_idx = indices.level1_indices[row_idx][subspace_idx] as usize;
                        let l1_centroid =
                            &codebooks.level1_codebooks[subspace_idx][l1_idx].centroid;

                        // Calculate residual
                        let residual: Vec<f32> = original_subspace
                            .iter()
                ...
                if !assigned_residuals.is_empty() {
                    let centroid =
                        &mut codebooks.level2_codebooks[subspace_idx][centroid_idx].centroid;
                    for dim in 0..centroid.len() {
...
mod tests {
    ...
    fn test_reconstruction() {
        ...
        let level2_codebooks = vec![vec![CodebookEntry {
        ...
        let reconstructed = refiner
            .reconstruct_from_codebooks(&codebooks, &indices, &weights)
            .unwrap();
