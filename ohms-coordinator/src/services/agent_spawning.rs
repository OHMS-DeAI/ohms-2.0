...
impl AgentSpawningService {
    ...
    pub async fn spawn_agents_from_instructions(
        ...
        let analysis =
            InstructionAnalyzerService::analyze_instructions(instructions, user_principal)?;
    ...
    async fn create_agent_instance(
        ...
        let canister_id = call_result
            .canister_id
            .ok_or_else(|| "No canister ID returned".to_string())?;
    ...
    /// Make cross-canister call to agent canister
    async fn call_agent_canister_create(
        config: AgentCreationConfig,
    ) -> Result<AgentCreationCallResult, String> {
    ...
    async fn store_spawning_result(result: &SpawningResult) -> Result<(), String> {
        ...
        with_state_mut(|state| {
            state
                ...
                .insert(result.request_id.clone(), agent_creation_result);
        });
