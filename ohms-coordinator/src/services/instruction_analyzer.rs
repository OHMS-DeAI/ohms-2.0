...
impl InstructionAnalyzerService {
    ...
    fn get_capability_patterns() -> Vec<CapabilityPattern> {
        vec![
            ...
            CapabilityPattern {
                keywords: vec!["code", "programming", "develop", "software", "application"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["coding", "software_development", "programming"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["code-llama", "starcoder", "wizardcoder"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
            ...
            CapabilityPattern {
                keywords: vec!["test", "testing", "qa", "quality", "verify"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["testing", "quality_assurance", "verification"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["code-llama", "starcoder"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
            ...
            CapabilityPattern {
                keywords: vec!["review", "code review", "peer review"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["code_review", "quality_assurance", "best_practices"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["code-llama", "starcoder"]
            ...
            CapabilityPattern {
                keywords: vec!["write", "content", "article", "blog", "documentation"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["content_creation", "writing", "documentation"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["llama", "mistral", "gemma"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
            ...
            CapabilityPattern {
                keywords: vec!["marketing", "social media", "campaign", "promote"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["marketing", "social_media", "campaign_management"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["llama", "mistral"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
            ...
            CapabilityPattern {
                keywords: vec!["analyze", "data", "analytics", "insights", "report"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["data_analysis", "analytics", "reporting"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["llama", "mistral", "gemma"]
            ...
            // Research patterns
            CapabilityPattern {
                keywords: vec!["research", "investigate", "study", "explore"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                capabilities: vec!["research", "investigation", "analysis"]
                    .into_iter()
                    .map(|s| s.to_string())
                    .collect(),
                model_suggestions: vec!["llama", "mistral", "gemma"]
    ...
    fn matches_pattern(instructions: &str, keywords: &[String]) -> bool {
        keywords
            .iter()
            .any(|keyword| instructions.contains(keyword))
    }
    ...
    fn check_user_quotas(
        ...
        with_state_mut(|state| {
            state
                .user_quotas
                .insert(user_principal.to_string(), user_quota.clone());
        });
    ...
    fn get_capabilities_for_specialization(specialization: &str) -> Vec<String> {
        match specialization {
            "Software Developer" => {
                vec!["coding", "software_development", "programming", "debugging"]
            }
...
mod tests {
    ...
    fn test_parse_instructions_development() {
        ...
        assert!(parsed
            .required_capabilities
            .contains(&"software_development".to_string()));
        assert!(parsed
            .specializations
            .contains(&"Software Developer".to_string()));
    ...
    fn test_parse_instructions_content_creation() {
        ...
        assert!(parsed
            .required_capabilities
            .contains(&"content_creation".to_string()));
        assert!(parsed
            .required_capabilities
            .contains(&"writing".to_string()));
        assert!(parsed
            .specializations
            .contains(&"Content Creator".to_string()));
        assert!(parsed.agent_count >= 1);
    }
    ...
    fn test_parse_instructions_complex_team() {
        let instructions =
            "Build a complex software system with a team of developers, testers, and reviewers";
        let parsed = InstructionAnalyzerService::parse_instructions(instructions).unwrap();

        assert!(parsed.agent_count >= 3);
        assert!(
            parsed.complexity_level == ComplexityLevel::Complex
                || parsed.complexity_level == ComplexityLevel::Enterprise
        );
