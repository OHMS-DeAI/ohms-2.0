...
use crate::config::llm_providers::execute_llm;
use crate::services::{with_state, with_state_mut};
use ic_cdk::api::time;
use ohms_shared::llm_client::LlmRequest;
...
impl OrchestrationService {
    ...
    pub fn promote_queen(task_id: &str) -> Result<String, String> {
        with_state_mut(|state| {
            ...
            let best_agent = if state.agent_capabilities.is_empty() {
                ...
                state
                    .agent_capabilities
                    .insert(queen_id.clone(), capabilities);
                queen_id
            } else {
            ...
            if let Some(role_map) = state.agent_roles.get_mut(&best_agent) {
                *role_map = AgentRole::Queen;
                ...
                state
                    .agent_roles
                    .insert(best_agent.clone(), AgentRole::Queen);
            }
    ...
    fn select_best_queen_candidate(
    ...
    pub fn assign_workers(task_id: &str, worker_count: usize) -> Result<Vec<String>, String> {
        with_state_mut(|state| {
            ...
            let mut available_agents: Vec<String> = state
                .agent_capabilities
                .keys()
            ...
            for agent_id in &available_agents {
                state
                    .agent_roles
                    .insert(agent_id.clone(), AgentRole::Worker);
            }
            ...
            if let Some(task) = state.orchestration_tasks.get_mut(&task_id) {
                ...
                if quality_score >= task.quality_threshold {
                    ...
                    task.error_message = Some(
                        "Max iterations reached without meeting quality threshold".to_string(),
                    );
                    task.completed_at = Some(time());
                } else {
    ...
    async fn queen_plan_iteration(
        ...
        let response = execute_llm(&request).await?;
    ...
    fn parse_plan_into_subtasks(plan: &str) -> Result<Vec<Subtask>, String> {
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if !trimmed.is_empty()
                && (trimmed.starts_with('-') || trimmed.starts_with(&format!("{}.", idx + 1)))
            {
    ...
    async fn worker_execute_subtask(
        worker_id: &str,
        subtask: &Subtask,
        ...
        for (i, exec) in executions.iter().enumerate() {
            ...
            if let Some(next_exec) = executions.get(i + 1) {
                ...
                messages.push(PeerMessage {
                    ...
                    content: format!(
                        "I've completed: {}. This might help with your task.",
                        exec.result.chars().take(100).collect::<String>()
                    ),
                    timestamp: time(),
                });
            }
