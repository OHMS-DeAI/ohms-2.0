use crate::domain::*;
use crate::services::{with_state, with_state_mut, DedupService, RegistryService};
...
impl RoutingService {
    pub async fn route_request(request: RouteRequest) -> Result<RouteResponse, String> {
        ...
        let selected_agents = match request.routing_mode {
            ...
            RoutingMode::Broadcast => {
                Self::select_multiple_agents(&request.capabilities_required, 3)?
            }
            RoutingMode::AgentSpawning => {
                Self::select_spawning_agents(&request.capabilities_required, 5)?
            }
        };
        ...
        with_state_mut(|state| {
            ...
            let new_avg = (state.metrics.average_routing_time_ms
                * (state.metrics.total_routes - 1) as f64
                + routing_time_ms as f64)
                / state.metrics.total_routes as f64;
    ...
    pub async fn fanout_best_result(
        ...
        let futures = agents.iter().map(|agent| {
            ...
            async move {
                ...
                let (result,): (AResult2,) = call(pr, "infer", (req,))
                    .await
                ...
                let scored = match result {
                    AResult2::Ok(resp) => {
                        ...
                        let score = Self::score_response(&resp, elapsed)
                            + if evidence.passed { 0.1 } else { 0.0 };
    ...
    pub fn get_stats(agent_id: Option<String>) -> Vec<RoutingStats> {
        with_state(|state| match agent_id {
    ...
    fn run_verifiers(resp: &AInferenceResponse) -> VerifierEvidence {
        ...
        if resp.generated_text.trim_start().starts_with('{') {
            ...
            if !has_colon {
                return VerifierEvidence {
                    passed: false,
                    details: "invalid json shape".to_string(),
                };
            }
        }
        VerifierEvidence {
            passed: true,
            details: "basic checks pass".to_string(),
        }
    }
}
