...
use ohms_shared::{
    ...
    current_time_seconds, current_time_millis, novaq::NOVAQIntegration
};
use ohms_adaptq::{NOVAQCompressor, CompressionConfig, CompressionResult, ModelFormat};
...
pub async fn upload_model(request: ModelUploadRequest) -> OHMSResult<ModelUploadResponse> {
    ...
    if request.name.is_empty() {
        return Err(OHMSError::InvalidInput("Model name cannot be empty".to_string()));
    }
    
    if request.data.is_empty() {
        return Err(OHMSError::InvalidInput("Model data cannot be empty".to_string()));
    }
    ...
    if MODELS.with(|models| models.borrow().contains_key(&model_id)) {
        return Err(OHMSError::AlreadyExists(format!("Model {} already exists", model_id)));
    }
...
pub async fn compress_model(
    ...
    let model_info = MODELS.with(|models| {
        models.borrow().get(&model_id).ok_or_else(|| {
            OHMSError::NotFound(format!("Model {} not found", model_id))
        })
    })?;
    
    if model_info.owner != caller_id {
        return Err(OHMSError::Unauthorized("Only model owner can compress".to_string()));
    }
    
    if model_info.status == ModelStatus::Compressing {
        return Err(OHMSError::InvalidState("Model is already being compressed".to_string()));
    }
...
pub async fn deploy_model(model_id: String) -> OHMSResult<()> {
    ...
    if model_info.owner != caller_id {
        return Err(OHMSError::Unauthorized("Only model owner can deploy".to_string()));
    }
    
    if model_info.status != ModelStatus::Uploaded && model_info.status != ModelStatus::Compressed {
        return Err(OHMSError::InvalidState("Model must be uploaded or compressed before deployment".to_string()));
    }
...
pub async fn delete_model(model_id: String) -> OHMSResult<()> {
    ...
    if model_info.owner != caller_id {
        return Err(OHMSError::Unauthorized("Only model owner can delete".to_string()));
    }
...
pub fn get_compression_job_status(job_id: String) -> OHMSResult<CompressionJob> {
    COMPRESSION_JOBS.with(|jobs| {
        jobs.borrow().get(&job_id).ok_or_else(|| {
            OHMSError::NotFound(format!("Compression job {} not found", job_id))
        })
    })
}
...
pub async fn run_inference(request: InferenceRequest) -> OHMSResult<InferenceResponse> {
    ...
    let model_info = MODELS.with(|models| {
        models.borrow().get(&request.model_id).ok_or_else(|| {
            OHMSError::NotFound(format!("Model {} not found", request.model_id))
        })
    })?;
    
    if model_info.status != ModelStatus::Deployed {
        return Err(OHMSError::InvalidState("Model must be deployed for inference".to_string()));
    }
    ...
    let output_data = perform_model_inference(&request.model_id, &request.input_data, &request).await?;
...
pub async fn terminate_inference_session(session_id: String) -> OHMSResult<()> {
    ...
    let mut session = INFERENCE_SESSIONS.with(|sessions| {
        sessions.borrow().get(&session_id).ok_or_else(|| {
            OHMSError::NotFound(format!("Session {} not found", session_id))
        })
    })?;
    
    if session.requester != caller_id {
        return Err(OHMSError::Unauthorized("Only session owner can terminate".to_string()));
    }
    ...
    if let Some(coordinator) = coordinator_id {
        let canister_info = CanisterInfo {
            ...
            metadata: {
                ...
                metadata.insert("model_count".to_string(), 
...
async fn validate_and_update_session(session_id: &str, requester: Principal) -> OHMSResult<()> {
    ...
    if session.requester != requester {
        return Err(OHMSError::Unauthorized("Session does not belong to caller".to_string()));
    }
    ...
    COMPRESSION_JOBS.with(|jobs| {
        ...
        for (job_id, mut job) in jobs.borrow_mut().iter() {
            if job.model_id == model_id && 
               (job.status == CompressionStatus::Queued || job.status == CompressionStatus::InProgress) {
...
async fn notify_coordinator_model_upload(model_id: &str, model_name: &str) -> OHMSResult<()> {
    if let Some(coordinator_id) = get_coordinator_canister_id() {
        ...
        match result {
            Ok((Ok(()),)) => Ok(()),
            Ok((Err(e),)) => Err(e),
            Err(e) => Err(OHMSError::CommunicationFailed(format!("Coordinator call failed: {:?}", e))),
        }
    } else {
