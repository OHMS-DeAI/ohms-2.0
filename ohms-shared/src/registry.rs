...
use ic_stable_structures::{StableBTreeMap, memory::Memory, DefaultMemoryImpl};
...
impl CanisterRegistry {
    ...
    pub fn register_canister(&mut self, info: CanisterInfo) -> OHMSResult<()> {
        ...
        self.canister_ids_by_type.insert(info.canister_type.clone(), info.canister_id);
    ...
    pub fn update_health_status(&mut self, canister_id: &str, status: CanisterStatus, health_score: f32) -> OHMSResult<()> {
        if let Some(mut info) = self.canisters.get(canister_id) {
            ...
            Err(OHMSError::NotFound(format!("Canister {} not registered", canister_id)))
        }
    }
    ...
    pub async fn health_check_all(&mut self) -> Vec<(String, CanisterStatus)> {
        ...
        for (canister_id, info) in self.canisters.iter() {
            ...
            self.update_health_status(&canister_id, status.clone(), 
                match status {
        ...
        match canister_status(CanisterIdRecord { canister_id: *canister_id }).await {
            Ok((response,)) => {
                match response.status {
                    ic_cdk::api::management_canister::main::CanisterStatusType::Running => CanisterStatus::Healthy,
                    ic_cdk::api::management_canister::main::CanisterStatusType::Stopping => CanisterStatus::Degraded,
                    ic_cdk::api::management_canister::main::CanisterStatusType::Stopped => CanisterStatus::Stopped,
                }
            },
    ...
    pub fn bootstrap_local_canisters(&mut self) -> OHMSResult<()> {
        ...
        self.register_canister(CanisterInfo {
            canister_id: Principal::from_text(&model_id).map_err(|_| OHMSError::InvalidInput("Invalid model canister ID".to_string()))?,
        ...
        self.register_canister(CanisterInfo {
            canister_id: Principal::from_text(&agent_id).map_err(|_| OHMSError::InvalidInput("Invalid agent canister ID".to_string()))?,
        ...
        self.register_canister(CanisterInfo {
            canister_id: Principal::from_text(&coordinator_id).map_err(|_| OHMSError::InvalidInput("Invalid coordinator canister ID".to_string()))?,
        ...
        self.register_canister(CanisterInfo {
            canister_id: Principal::from_text(&econ_id).map_err(|_| OHMSError::InvalidInput("Invalid econ canister ID".to_string()))?,
    ...
    fn read_canister_id_from_file(&self, canister_name: &str) -> Result<String, std::io::Error> {
        ...
        match canister_name {
            ...
            _ => Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Canister not found")),
        }
    }
...
pub fn discover_canisters_from_dfx() -> OHMSResult<HashMap<String, Principal>> {
    ...
    canisters.insert("ohms_econ".to_string(), 
